...
public class AXmlResourceParser implements XmlResourceParser {
    ...
    private String added_appNamespace;
...
//        }
        ...
        if (value == null || value.length() == 0) {
            // Attr name is like 0x7fxxxxxx, revise the namespace
            int name = m_attributes[offset + ATTRIBUTE_IX_NAME];
            if (m_resourceIDs != null && name >= 0 && name < m_resourceIDs.length
                    && (m_resourceIDs[name] & 0x7f000000) == 0x7f000000) {
                String appNamespace = getPossibleNamespace();
                if (appNamespace != null) {
                    value = appNamespace;
                }
            }
            // Original logical from apktool
            else if (m_strings.getString(name).length() == 0) {
                value = android_ns;
            }
        }
    ...
    private String getPossibleNamespace() {
        if (added_appNamespace != null) {
            return added_appNamespace;
        }
        for (int i = 0; i < m_strings.getCount(); i++) {
            String name = m_strings.getString(i);
            if (name != null && name.startsWith("http://") && !name.equals(android_ns)) {
                added_appNamespace = name;
                break;
            }
        }
        return added_appNamespace;
    ...
    public String getAttributeName(int index) {
        ...
        // Pujiang modified: for protected apk, first try to get it from
        // resource id
        if (apkProtected) {
            try {
                int resId = getAttributeNameResource(index);
                if (resId != 0) {
                    String value = mAttrDecoder.decodeManifestAttr(resId);
                    return value;
                }
...
//                        getAttributeNameResource(index));
                // Modified by Pujiang: make it use the valueData, not valueRaw, not sure work or not
                String rawValue = (valueRaw == -1 ? null :
                        ResXmlEncoders.escapeXmlChars(m_strings.getString(valueRaw)));
                if (apkProtected) { // When protected, use valueData, not valueRaw
                    if ((valueData & 0x7f000000) == 0x7f000000) {
                        rawValue = null;
                    }
                }
                String ret = mAttrDecoder.decode(
                        valueType,
                        valueData,
                        rawValue,
                        getAttributeNameResource(index));
                ...
                setFirstError(ex);
                LOGGER.log(Level.WARNING, String.format("Could not decode attr value, using raw value "
