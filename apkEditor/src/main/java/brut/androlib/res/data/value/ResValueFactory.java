...
public class ResValueFactory {
    ...
    public ResScalarValue factory(int type, int value, String rawValue) throws AndrolibException {
        ...
        throw new AndrolibException("Invalid value type: " + type + ", value: " + value);
    }

    public ResIntBasedValue factory(String value, int rawValue, boolean isFileValue,
                                    String rawStr, boolean isStyled) {
        // Modified by Pujiang
        if (value == null || value.equals("")) {
            return new ResStringValue(value, rawValue);
        } else if (isFileValue || value.startsWith("res/")
                || value.startsWith("r/") || value.startsWith("R/")) {
            // Pujiang: character like '#' (in URET.apk) be escaped is not correct
            //return new ResFileValue(value, rawValue);
            return new ResFileValue(rawStr, rawValue);
        } else {
            return new ResStringValue(value, rawValue, rawStr, isStyled);
        }
