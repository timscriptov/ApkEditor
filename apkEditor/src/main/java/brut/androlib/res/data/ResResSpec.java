...
public class ResResSpec {
    ...
    // Modified by Pujiang for performance
    public ResResSpec(ResID id, String name, ResPackage pkg, ResTypeSpec type, boolean apkProtected) {
        this.mId = id;
        String cleanName;

        if (apkProtected) {
            try {
                type.getResSpec(name);
                cleanName = name + "_dup_" + id.toString();
            } catch (AndrolibException ex) {
                cleanName = ((name == null || name.isEmpty()) ? ("noname_" + id.toString()) : name);
            }
        } else {
            cleanName = ((name == null || name.isEmpty()) ? ("noname_" + id.toString()) : name);
        }
    ...
    public boolean isDummyResSpec() {
        return mName.startsWith("dummy_ae_");
    }
