...
public class KXmlSerializer implements XmlSerializer {
    ...
    private static void reportInvalidCharacter(char ch) {
        ...
        Log.e("APKEDITOR", "Illegal character (U+" + Integer.toHexString((int) ch) + ")");
    }
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            switch (c) {
                ...
                case '\t':
                    ...
                    break;
                case '&':
                    if (i + 4 < s.length() && s.charAt(i + 1) == 'a'
                            && s.charAt(i + 2) == 'm' && s.charAt(i + 3) == 'p'
                            && s.charAt(i + 4) == ';') {
                        i += 4;
                        append("&amp;");
                    } else if (i + 3 < s.length() && s.charAt(i + 1) == 'l'
...
//                    }
    ...
    */
    ...
    public XmlSerializer text(String text) throws IOException {
        ...
        // Added by Pujiang
//        if (text.indexOf('<') != -1) {
//            List<StringPattern.TagRecord> tags = StringPattern.getHtmlTags(text);
//            if (tags != null && !tags.isEmpty()) {
//                // For html string like "<font color=#333333>", it contains blank char
//                boolean blankContained = false;
//                for (StringPattern.TagRecord tag : tags) {
//                    String htmlStr = text.substring(tag.startPos, tag.endPos);
//                    if (htmlStr.contains(" ")) {
//                        blankContained = true;
//                        break;
//                    }
...
//                int start = 0;
//                for (StringPattern.TagRecord tag : tags) {
//                    writeEscaped(text.substring(start, tag.startPos), -1); // text before the tag
//                    String htmlStr = text.substring(tag.startPos, tag.endPos);
//                    // Write tag itself
//                    if (blankContained) {
//                        directWrite(htmlStr.replace("<", "&lt;").replace("'", "\\'"));
//                    } else {
//                        directWrite(htmlStr);
//                    }
//                    start = tag.endPos;
//                }
//                writeEscaped(text.substring(start), -1); // remaining string
//                return this;
////                if (text.indexOf('\n') != -1) {
////                    directWrite("<![CDATA[" + text + "]]>");
////                } else {
////                    directWrite("<![CDATA[" + text + "]]>");
////                }
////                return this;
...
//        }
    ...
    // Added by Pujiang
    private void directWrite(String text) throws IOException {
        int i = 0;
        ...
        while (length > 0) {
            ...
            text.getChars(i, i + batch, mText, mPos);
            i += batch;
            length -= batch;
            mPos += batch;
        }
