...
public class StringBlock {
    ...
    private SparseArray<String> newValues = new SparseArray<>();
    /**
    ...
    public static void skipCheckChunkTypeInt(IntReader reader, int expected, int possible) throws IOException {
        int got = reader.readInt();

        if (got == possible || got < expected) {
            skipCheckChunkTypeInt(reader, expected, -1);
        } else if (got != expected) {
            throw new IOException(String.format("Expected: 0x%08x, actual: 0x%08x", expected, got));
        }
    }
    ...
    public static StringBlock read(IntReader reader) throws IOException {
        skipCheckChunkTypeInt(reader, CHUNK_STRINGPOOL_TYPE, CHUNK_NULL_TYPE);
    ...
    public String getString(int index) {
        String assignedVal = newValues.get(index);
        if (assignedVal != null) {
            return assignedVal;
        }
    ...
    public void setString(int idx, String newVal) {
        newValues.put(idx, newVal);
    }
    ...
    public String getHtml(int index) {
        ...
        return getHTML(index, raw);
    }

    public String getHTML(int index, String raw) {
        if (raw == null) {
            return raw;
        }
        ...
        while (true) {
            ...
            for (j = depth - 1; j >= 0; j--) {
                ...
                if (offset <= end) {
                    ...
                    html.append(escapeXmlChars(safeSubstring(raw, offset, end + 1)));
                    offset = end + 1;
                }
            ...
            if (offset < start) {
                ...
                html.append(escapeXmlChars(safeSubstring(raw, offset, start)));
            ...
            if (!close) {
                ...
                while (loop) {
                    ...
                    if (pos2 != -1) {
                        ...
                        builder.append(escapeXmlChars(val)).append('"');
                    } else {
                        loop = false;
                    }
